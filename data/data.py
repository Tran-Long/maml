...
class MetaDataset:
    def __init__(self, mode="train", config_file=Path(__file__).parent/"config.yaml") -> None:
        ...
        for image_path in all_images_list:
            character_name = image_path.parent.name
    ...
    def sample_task(self):
        ...
        for pseudo_cls, char_idx in enumerate(selected_character_indices):
            ...
            selected_image_indices = rng.choice(len(self.images_dict[self.character_names[char_idx]]), self.k_shots + meta_test_per_class, replace=False)
